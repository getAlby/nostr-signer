// modifed from https://github1s.com/nostr-connect/connect/blob/master/src/connect.ts

import EventEmitter from "events";
import { Event, getPublicKey, Kind } from "nostr-tools";
...
export class Connect {
  ...
  async init() {
    ...
    sub.on("event", async (event: Event) => {
      ...
      /* eslint-disable @typescript-eslint/no-unused-vars */
      try {
        const plaintext = await new Nip04(this.rpc.self.secret).decrypt(
          event.pubkey,
          event.content
        );
